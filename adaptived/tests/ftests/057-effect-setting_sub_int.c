 ...
 */
/**
 * Test to subtract from an integer setting
 *
 */
...
static const char * const setting_file = "./test057.setting";
static const int expected_value = 22222222;
...
int main(int argc, char *argv[])
{
	...
	snprintf(config_path, FILENAME_MAX - 1, "%s/057-effect-setting_sub_int.json", argv[1]);
	...
	write_file(setting_file, "44444444");
	...
	if (ret != EXPECTED_RET) {
		adaptived_err("Test 057 returned: %d, expected: %d\n", ret, EXPECTED_RET);
		goto err;
	}
