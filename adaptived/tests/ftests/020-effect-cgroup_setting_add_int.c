...
static const char * const cgroup_file = "./test020.cgroup";
static const int expected_value = 627;
...
int main(int argc, char *argv[])
{
	...
	snprintf(config_path, FILENAME_MAX - 1, "%s/020-effect-cgroup_setting_add_int.json", argv[1]);
	...
	write_file(cgroup_file, "100");

	ret = adaptived_set_attr(ctx, ADAPTIVED_ATTR_MAX_LOOPS, 500);
	if (ret)
		goto err;
