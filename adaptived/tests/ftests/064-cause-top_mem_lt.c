...
static const char * const setting_file = "064-cause-top_mem_lt.setting";
static int ctr = 0;
...
static long long value[] = { 527700400, 527700380, 527700360, 527700340, 527700320, 527700300, 527700280 };
long long expected_value = 527700320;
...
static int inject(struct adaptived_ctx * const ctx)
{
	...
	w = sprintf(buf, "MemTotal:       %lld kB\n", value[ctr]);
	if (w <= 0 || w >= FILENAME_MAX) {
		ret = -errno;
		goto err;
	}
	strcpy(&buf[strlen(buf)], "MemFree:        488784560 kB\n");
...
static int validate_meminfo(const char *meminfo_file, long long expected_value)
{
	...
	while ((read = getline(&line, &len, fp)) != -1) {
		...
		switch (i) {
		case 0:
			sprintf(buf, "MemTotal:       %lld kB", expected_value);
			buf[FILENAME_MAX - 1] = '\0';
			pat = buf;
			break;
		case 1:
			pat = "MemFree:        488784560 kB";
			break;
		case 2:
