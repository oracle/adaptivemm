 ...
 */
/**
 * Test to build and load a rule at runtime
 *
 */
...
#define EXPECTED_RET -84

static const char * const rule_name = "test 015";
...
int main(int argc, char *argv[])
{
	...
	eff = adaptived_build_effect("validate");
	if (!eff)
		goto err;

	ret = adaptived_effect_add_int_arg(eff, "return_value", -EXPECTED_RET);
	if (ret)
		goto err;
	...
	/* Let the adaptived_loop() run a couple times before loading our rule */
	sleep(2);
	...
	if (tret != (void *)EXPECTED_RET)
		goto err;

	ret = adaptived_get_rule_stats(ctx, rule_name, &stats);
	if (ret)
		goto err;
	if (stats.loops_run_cnt != 1)
		goto err;
