 ...
 * that config with just date format works.
 *
 */
...
static const char * const test_file1 = "test044_test_file1.txt";
static const char * const test_file2 = "test044_test_file2.txt";
...
int main(int argc, char *argv[])
{
	...
	/* test with just date format */
	setenv("TZ", "Pacific/Honolulu", 1);
	snprintf(config_path, FILENAME_MAX - 1, "%s/044-effect-logger-date-format.json", argv[1]);
	...
	strcpy(expected_buf1, "This was written to test044_test_file1.txt");
	write_file(test_file1, expected_buf1);
	strcpy(expected_buf2, "This was written to test044_test_file2.txt");
	...
	if (ret != EXPECTED_RET) {
		adaptived_err("Test 044 returned: %d, expected: %d\n", ret, EXPECTED_RET);
		goto err;
	}
	fp = fopen(log_file, "r");
	if (fp == NULL) {
	...
	while ((read = getline(&line, &len, fp)) != -1) {
		...
		case 16:
			if (!strstr(line, "-HST")) {
				adaptived_err("separator failure, missing -HST\n");
				goto err;
			}
			lines_found++;
			break;
