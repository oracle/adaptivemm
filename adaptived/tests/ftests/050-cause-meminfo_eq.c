...
static const char * const setting_file = "050-cause-meminfo_eq.setting";
static int ctr = 0;
static bool non_kb = false;
...
static long long value[] = { 493452940, 493452941, 493452942, 493452943, 493452944, 493452945, 493452946 };
long long expected_value = 493452945;
...
static int inject(struct adaptived_ctx * const ctx)
{
	...
	if (non_kb)
		w = sprintf(&buf[strlen(buf)], "MemFree:        %lld\n", value[ctr] * 1024);
	else
...
static int validate_meminfo(const char *meminfo_file, long long expected_value)
{
	...
	while ((read = getline(&line, &len, fp)) != -1) {
		...
		case 1:
			if (non_kb)
				sprintf(buf, "MemFree:        %lld", expected_value);
			else
...
int main(int argc, char *argv[])
{
	...
	/* Test a meminfo value that ends in kB */
	snprintf(config_path, FILENAME_MAX - 1, "%s/050-cause-meminfo_eq.json", argv[1]);
	...
	/* Test a meminfo value that does not end in kB */
	ctr = 0;
	non_kb = true;
	...
	ret = validate_meminfo(setting_file, expected_value * 1024);
	if (ret)
		goto err;
